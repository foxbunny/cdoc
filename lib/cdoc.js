...
var VERBOSE, a, buildToc, cdoc, commentRe, empty, emptyLineRe, existsSync, extract, headingRe, indentedRe, isDir, isGlob, isSkippable, iter, log, matchGlob, mkdirSync, opts, processDir, processFile, processParagraph, processParagraphs, readFileSync, readdirSync, slug, statSync, toArray, tocIntend, wrap, writeFileSync, _ref, _ref1;
...
cdoc.extract = extract = function(filename) {
...
cdoc.processParagraph = processParagraph = function(_arg) {
...
cdoc.processParagraphs = processParagraphs = function(paragraphs) {
  var p;
  return ((function() {
...
cdoc.procesFile = processFile = function(filename) {
  var mkd, paragraphs;
...
cdoc.log = log = function(msg) {
  if (VERBOSE) {
    return console.log(msg);
  }
};

cdoc.isDir = isDir = function(path) {
  return statSync(path).isDirectory();
};

cdoc.isGlob = isGlob = function(s) {
  return s.indexOf('*') > -1 || s.indexOf('?') > -1;
};

cdoc.matchGlob = matchGlob = function(s, glob) {
  glob = glob.replace('*', '.*').replace('?', '.').replace('.', '\\.');
  return s.match(new RegExp(glob));
};

cdoc.isSkippable = isSkippable = function(filename, skipList) {
  var skip, t, _i, _len;
...
cdoc.processDir = processDir = function(dir, target, skip) {
